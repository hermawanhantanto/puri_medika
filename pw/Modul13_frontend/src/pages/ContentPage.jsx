...
const ContentPage = () => {
  ...
  const handleThumbnail = async (content_id) => {
    try {
      const imageInput = document.getElementById(`thumbnail-${content_id}`);
      ...
      imageInput.addEventListener("change", async (event) => {
        try {
          ...
          if (!response) throw new Error("Failed update thumbnail");
          console.log(content_id);
          toast.success("Success update thumbnail");
                ...
                <td>
                  <Stack direction="horizontal" gap={2}>
                    ...
                    <Button
                      variant="success"
                      type="button"
                      onClick={() => handleThumbnail(content.id)}
                    >
                      <FaImage /> Pilih Thumbnail
                      <input
                        type="file"
                        name={`thumbnail-${content.id}`} // Gunakan content.id sebagai bagian dari nama input
